 ...
 * 제목 , 내용 , 작성자를 입력 받아서 저장 출력 삭제 수정을 구현 할 수 있다
 * 게시물 3개 저장 하기 , 게시물 1개당 제목/내용/작성자 저장소(변수) 3개 필요 , 게시물3개에 필요한 변수는 9개이다 
 */
public class TestBoardProgram2501145 {
	public static void main(String[] args) {
		
		//[6] 변수선언 , 기본타입 [불 차 바 쇼 인롱 플 더] 참조타입 [String,Scanner 등등]
		...
		while(true) {
			...
			int 선택메뉴 = scan.nextInt(); // 임의 정의한 메뉴번호(int) 입력받고 입력받은 값 저장하기
			
			//[4] 연산자 //[5] 조건문 , if(조건문){ }
			if(선택메뉴 == 1) {System.out.println("1.등록 선택");
			System.out.println("제목"); String title = scan.next(); // 메시지(print) > 입력(next) > 저장(변수)
			System.out.println("내용"); String content = scan.next(); // 메시지(print) > 입력(next) > 저장(변수)
			System.out.println("작성자"); String writer = scan.next(); // 메시지(print) > 입력(next) > 저장(변수)
				
			//[7-1] 변수값에 따른 데이터 검사 후 대입하기 // [6-2] 변수값 수정하기 // while 밖 변수에 입력받은 3개의 문자열을 각각 저장.
			...
			else if(제목3 == null) { 제목3 =title; 내용3 =content; 작성자3 = writer;} // 두번째 게시물이 비어 있으면
			else {System.out.println("** 등록실패 : 공간 부족 **");}
  // etc // == null : 비어있는지 확인 // != null : 글이 있는지 확인	
					
			} //ie
			
			//[7-2] 변수값에 따른 데이터 검사 후 출력하기 // [6-3] 변수값 호출하기 // 연결연산자 : "문자열" + 변수명 + "문자열" + 변수명
